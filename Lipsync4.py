"""
LipSync Analyzer v5.0 — With WhisperX Phoneme Alignment
...
def char_to_viseme(ch):
    """Convert character to viseme (uk/ru/en support)"""
    ch = ch.lower()
    if ch in "aaeUayo":          return "A"
    if ch in "eEe":              return "E"
    if ch in "iIiI":             return "I"
    if ch in "oOo":              return "O"
    if ch in "uUu":              return "U"
    if ch in "mpbMPB":           return "M"
    if ch in "fvFV":             return "F"
    if ch in "szSSZZ":           return "S"
    if ch in "lrLR":             return "L"
    if ch in " \t\n":            return "CLOSED"
    return "T"
...
def char_to_viseme(ch):
    ...
    ch = ch.lower()
    # Vowels
    if ch in "aaeuyayu":         return "A"
    if ch in "ee":               return "E"
    if ch in "iiyi":             return "I"
    if ch in "oo":               return "O"
    if ch in "uuyu":             return "U"
    # Bilabial stops
    if ch in "mpbmpb":           return "M"
    # Labiodental
    if ch in "fvfv":             return "F"
    # Sibilants
    if ch in "szsz":             return "S"
    # Liquids
    if ch in "lrlr":             return "L"
    # Spaces -> closed mouth
    if ch in " \t\n":            return "CLOSED"
...
def char_to_viseme(ch):
    ...
    vowels_a = set("aаяAАЯ")
    vowels_e = set("eеєEЕЄ")
    vowels_i = set("iіиїIІИЇ")
    vowels_o = set("oоOО")
    vowels_u = set("uуюUУЮ")
    bilabial  = set("mpbмпбMPBМПБ")
    labio     = set("fvфвFVФВ")
    sibilant  = set("szшщцчсзSZШЩЦЧСЗ")
    liquid    = set("lrлрLRЛР")
...
class LipSyncApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Auto LipSync v5.0 (WhisperX)")
    ...
    def _check_deps(self):
        ...
        if not self._ffmpeg_ok:
            ...
            self._run_btn.configure(state="disabled", text="[X] FFmpeg missing")
        if not self._whisperx_ok:
            issues.append("[X] WhisperX not installed!")
            issues.append("    Install: pip install whisperx")
            self._run_btn.configure(state="disabled", text="[X] WhisperX missing")
        if issues:
            self._log("\n".join(issues) + "\n\n", "err")
        else:
            self._log("[OK] v5.0 - WhisperX ready!\n", "ok")
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="LIPSYNC v5.0  (WhisperX)",
    ...
    def _fill_ui(self, root):
        ...
        sty = ttk.Style()
        sty.theme_use("clam")
        sty.configure("TCombobox", fieldbackground=BG, background=BTN_BG,
