...
def step_check_python():
    ...
    if v.major == 3 and v.minor >= 14:
        ...
        warn("Также будет установлена ПОСЛЕДНЯЯ доступная версия PyTorch (вместо фиксированной 2.1.2).")
    else:
...
def step_install_torch(cuda_ver):
    ...
    # === ИСПРАВЛЕНИЕ: убрана жёсткая версия 2.1.2 ===
    # Python 3.14 + CUDA 12.6 требуют torch 2.10+ (или 2.12+), которого нет в старом индексе 2.1.2
    if index_url:
